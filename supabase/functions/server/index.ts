...
app.get("/content/:section", async (c) => {
  try {
    const section = c.req.param('section');
    console.log(`📖 GET /content/${section}`);
...
app.post("/content/:section", requireAuth, async (c) => {
  try {
    const section = c.req.param('section');
    console.log(`✏️ POST /content/${section}`);
...
app.post("/analytics/pageview", async (c) => {
  try {
    ...
    console.log(`📊 POST /analytics/pageview - page: ${page}`);
...
app.post("/analytics/download", async (c) => {
  try {
    const { fileName, language } = await c.req.json();
    console.log(`💾 POST /analytics/download - file: ${fileName}`);
...
app.get("/analytics/dashboard", requireAuth, async (c) => {
  try {
...
app.get("/", (c) => {
  console.log(`🏠 GET /`);
  return c.json({ 
    message: "Fernando's Portfolio API", 
    version: "2.0",
    endpoints: [
      "GET /health",
      "GET /test",
      "GET /content/:section",
      "POST /content/:section (auth required)",
